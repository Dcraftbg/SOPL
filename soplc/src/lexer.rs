...
impl OfP {
    ...
    pub fn var_type(&self, build: &BuildProgram, local_vars: &Vec<HashMap<String, LocalVariable>>, buffers: &Vec<BuildBuf>) -> Option<VarType> {
        match self {
            ...
            Self::RESULT(f, _) => if let Some(f) = build.functions.get(f) { f.contract.Output.clone() } else if let Some(f) = build.externals.get(f) { f.contract.Output.clone() }else {None},
            Self::BUFFER(i) => {
                Some(VarType::PTR(Ptr::ref_to(buffers[i.to_owned()].typ.clone())))
            },
